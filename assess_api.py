...
async def assess_stuttering(file: UploadFile = File(...)):
    ...
    if "." not in name:
        raise HTTPException(400, "File needs an extension.")
    ...
    # 3) Sliding-window stutter detection (200 ms window, 100 ms hop)
    WIN, HOP, SR = 0.2, 0.1, 16_000
    wlen = int(WIN * SR)
    hop  = int(HOP * SR)
    ...
    for start in range(0, len(audio_np) - wlen + 1, hop):
        ...
        pred_labels.append(label_map[pid])

    # 4) Count events & total stutter time
    num_events     = 0
    stutter_frames = 0
    prev           = "none"
    ...
    # 5) Silence‐based “word” segmentation
    #    now using 100 ms min silence and 10 dB below avg level
    nonsilent_ms = detect_nonsilent(
        seg,
        min_silence_len=100,             # 100 ms
        silence_thresh=seg.dBFS - 10      # ~10 dB below average
    )
    word_intervals = [(start/1000.0, end/1000.0) for start, end in nonsilent_ms]
    total_words    = max(len(word_intervals), 1)

    # 6) Map stutter windows → word intervals
    ...
    # 7) Simplified SSI-4 style score
    event_durations = [(f1 - f0 + 1) * frame_duration
                       for f0, f1 in [
                           (i, i) for i, l in enumerate(pred_labels) if l != "none"
                       ]]
